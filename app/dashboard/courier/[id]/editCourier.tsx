...
import { editCourier } from "@/action/courier";
import { UserDto } from "@/lib/dto/user";

export default function EditCourierForm({
  courierPromise,
}: {
  courierPromise: Promise<UserDto | null>;
}) {
  const courier = use(courierPromise);
  const [state, action, isPending] = useActionState(editCourier, null);
    ...
    <form action={action}>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold">Edit Akun Kurir</h2>
        ...
        <div className="grid grid-cols-1 gap-6">
            ...
            <Card>
              <CardContent className="space-y-4">
                <InputField
                  ...
                  defaultValue={state?.inputs?.name ?? courier?.name ?? ""}
                  required
                  autoComplete="name"
                  disabled={!isEditing}
                ...
                <InputField
                  ...
                  defaultValue={state?.inputs?.email ?? courier?.email ?? ""}
                  autoComplete="email"
                  disabled={!isEditing}
                ...
                <InputField
                  ...
                  defaultValue={state?.inputs?.phone ?? courier?.phone ?? ""}
                  required
                  autoComplete="tel"
                  disabled={!isEditing}
          ...
          <input type="hidden" name="oldPhone" value={courier?.phone} />
        </div>
      </div>
    </form>
  );
