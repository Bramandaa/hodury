...
import { ChangeEvent, use, useActionState, useState } from "react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { editCategory } from "@/action/category";
...
export default function EditCategoryForm({
  ...
  categoryPromise: Promise<CategoryDTO | null>;
  query: string;
}) {
  const category = use(categoryPromise);
  const [state, action, isPending] = useActionState(editCategory, null);
                  ...
                  category.products.map((product) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-500">
                        Harga: {formatPrice(product?.price ?? 0)}
                      </p>
                    </div>
                  ))
                ) : (
